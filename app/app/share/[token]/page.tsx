...
export default function SharePage() {
  ...
  if (notFound) {
    return (
      <div className="sp-root" style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 14 }}>
        <AlertTriangle size={36} color="#f59e0b" />
        <div style={{ fontSize: 18, fontWeight: 700 }}>Link not found</div>
        <div style={{ fontSize: 13, color: "rgba(255,255,255,0.4)", textAlign: "center", padding: "0 24px" }}>
  ...
  return (
    <div className="sp-root" style={{ backgroundImage: glow }}>
      ...
      <div className="sp-body">
        ...
        {/* KPI grid */}
        <motion.div className="sp-kpi-grid" variants={stagger} initial="hidden" animate="show">
          {[
            ...
            { label: "Due This Week",     value: String(dueWeek),                sub: "upcoming renewals",              Icon: Calendar,   accent: dueWeek > 0 ? "#f59e0b" : "#60a5fa" },
            { label: "Potential Savings", value: `$${pausedAmt.toFixed(2)}/mo`,  sub: `${paused.length} paused`,        Icon: RefreshCw,  accent: "#22c55e" },
          ].map((kpi) => (
            <motion.div key={kpi.label} variants={cardIn} className="sp-card sp-kpi-card">
              <div className="sp-kpi-icon" style={{ background: kpi.accent + "1a" }}>
        ...
        {/* Savings alert */}
        {pausedAmt > 0 && (
          <motion.div variants={cardIn} initial="hidden" animate="show" className="sp-alert">
        ...
        <div className="sp-main-grid">
          ...
          {/* Subscription list */}
          <motion.div variants={cardIn} initial="hidden" animate="show" className="sp-card">
                ...
                {[...upcoming, ...cancelled].map((sub, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      style={{ borderBottom: i < subs.length - 1 ? "1px solid rgba(255,255,255,0.06)" : "none" }}
                    >
                      {/* Logo */}
                      <div className="sp-sub-logo" style={{ background: groupColor + "20", color: groupColor }}>
                        {sub.logoUrl
          ...
          <div className="sp-sidebar">
            ...
            {/* Status donut */}
            <motion.div variants={cardIn} initial="hidden" animate="show" className="sp-card">
              <div className="sp-section-title" style={{ marginBottom: 14 }}>Status Overview</div>
              {subs.length > 0 && (() => {
                ...
                return (
                  <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
                    <div style={{ position: "relative", width: 82, height: 82, flexShrink: 0 }}>
                      <div style={{ width: 82, height: 82, borderRadius: "50%", background: donut }} />
                      <div style={{
                    ...
                    <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 9 }}>
                      {[
                        ...
                        <div key={row.label} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          ...
                          <span style={{ fontSize: 12.5, flex: 1, color: "rgba(255,255,255,0.65)" }}>{row.label}</span>
                          <span style={{ fontSize: 14, fontWeight: 800 }}>{row.count}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            {top3.length > 0 && (
              <motion.div variants={cardIn} initial="hidden" animate="show" className="sp-card">
                <div className="sp-section-title" style={{ marginBottom: 14 }}>Top Expenses</div>
                    ...
                    return (
                      <div key={sub.id}>
                        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                          ...
                          <span style={{ fontSize: 13, fontWeight: 700 }}>${sub.amount.toFixed(2)}</span>
                        </div>
            ...
            {upcoming.slice(0, 5).length > 0 && (
              <motion.div variants={cardIn} initial="hidden" animate="show" className="sp-card">
                <div className="sp-section-title" style={{ marginBottom: 14 }}>Next Renewals</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  {upcoming.slice(0, 5).map((sub) => {
                    ...
                    return (
                      <div key={sub.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                        <span style={{ fontSize: 13, fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{sub.name}</span>
                        <span style={{
                          fontSize: 12, fontWeight: urg ? 700 : 400, flexShrink: 0,
        ...
        <div className="sp-footer">
          <div style={{ fontSize: 11.5, color: "rgba(255,255,255,0.2)", marginBottom: 5 }}>
            Read-only snapshot · Data may not reflect real-time changes
          </div>
          <div style={{ fontSize: 12.5, color: "rgba(255,255,255,0.35)", fontWeight: 600 }}>
